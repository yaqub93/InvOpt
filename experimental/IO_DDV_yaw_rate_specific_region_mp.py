...
sample_indices = random.sample(range(len(dataset[feature_names[0]])), k=500)  # Change 'k' to the desired sample size
...
def process_N(args):
    ...
    return run, N_index, x_diff_train, x_diff_test, x_diffs_train, x_diffs_test, theta_IO
...
for N_results in results:
    ...
    x_diffs_train_hist[p_index, N_results[0], N_results[1], :] = N_results[4]
    x_diffs_test_hist[p_index, N_results[0], N_results[1], :] = N_results[5]
    theta_IOs.append(N_results[6])
...
final_results['x_diffs_train_hist'] = x_diffs_train_hist
final_results['x_diffs_test_hist'] = x_diffs_test_hist
