...
def FOP_QP(theta, s):
    ...
    mdl.setObjective(
        #Qyy*y**2 + y*gp.quicksum(Q[i]*phi1_w[i] for i in range(1)), gp.GRB.MINIMIZE
        Qyy*y**2, gp.GRB.MINIMIZE
    )
...
def load_data(train_test_slip):
    ...
    sample_indices = random.sample(range(len(dataset[feature_names[0]])), k=1000)  # Change 'k' to the desired sample size
