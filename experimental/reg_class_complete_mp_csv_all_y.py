...
def quadratic_FOP(theta, s):
    ...
    mdl.setObjective(
        ...
        + quicksum(qx[i]*w[i]*w[i] for i in range(t)),
        GRB.MINIMIZE
    )
...
def IO_preprocessing(X_train, X_test, constraint):
    """Preprocess data for IO."""
    #A = -np.eye(1)
    #B = np.zeros((1, 1))
    A = np.array([-1,1, -1,1, -1,1]).reshape((1,6))
...
def IO_preprocessing2(S_train, X_train, S_test, X_test, constraint):
    ...
    #A = -np.eye(1)
    #B = np.zeros((1, 1))
    A = np.array([-1,1, -1,1, -1,1]).reshape((6,1))
    B = np.array([np.abs(constraint[0])])
...
X_train, X_test, constraint = load_data(train_test_slip, num_sample = 100)
dataset_train, dataset_test = IO_preprocessing(
    X_train, X_test, constraint
)
...
#                                    reg_param=kappa)
#print(theta_IO)
#print(theta_IO/np.sum(theta_IO))
